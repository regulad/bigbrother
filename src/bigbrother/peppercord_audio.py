...
# Copied from regulad/discordnpc https://github.com/regulad/discordnpc/blob/master/discordnpc/peppercord_audio.py
...
class CustomVoiceClient(VoiceClient):
    ...
    def cleanup(self) -> None:
        """
        Clean up the VoiceClient and stop recording if we are recording.
        """
        try:
            self.stop_recording()
...
class FFmpegPCMAudioBytes(AudioSource):
    ...
    def __init__(
        self, source: bytes, *, executable="ffmpeg", pipe=True, stderr=None, before_options=None, options=None
    ):
...
BytesSource: TypeAlias = EnhancedFFmpegPCMAudioBytesTransformed

# We only export the useful classes here.
__all__ = ("BytesSource", "CustomVoiceClient")
